...
public class TransferServiceImpl implements TransferService {
    ...
    public void transfer(TransferRequest request) {
        Card fromCard = cardRepository.findByNumber(request.numberFrom())
                .orElseThrow(() -> new RuntimeException("Source card not found: " + request.numberFrom()));
        
        Card toCard = cardRepository.findByNumber(request.numberTo())
                .orElseThrow(() -> new RuntimeException("Destination card not found: " + request.numberTo()));
        
        // owner check
        if (!fromCard.getOwner().getUserId().equals(toCard.getOwner().getUserId())) {
            throw new RuntimeException("Cards must belong to the same user");
        }
        
        // status check
        if (fromCard.getStatus() != CardStatusEnum.ACTIVE) {
            throw new RuntimeException("Source card is not active. Status: " + fromCard.getStatus());
        }
        if (toCard.getStatus() != CardStatusEnum.ACTIVE) {
            throw new RuntimeException("Destination card is not active. Status: " + toCard.getStatus());
        }
